...
use hlagg::protocol::{dl::DiscreteLog, Aggregation};
...
// TODO: Tweak
const NUM_CLIENTS: [usize; 1] = [10000];
    ...
    for num_clients in NUM_CLIENTS {
        for length in LENGTHS {
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        black_box(Agg::encode(&cks[0], &input, &mut OsRng).unwrap());
                    });
                }
            );
        }
...
fn bench_verify(c: &mut Criterion) {
    ...
    for num_clients in NUM_CLIENTS {
        for length in LENGTHS {
            ...
            let encodings_and_proofs: Vec<_> = cks.iter().zip(inputs.iter())
                .map(|(ck, input)| Agg::encode(ck, input, &mut OsRng).unwrap())
                .collect();
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        black_box(Agg::verify_encodings(&params, None, &encodings, &proofs).unwrap());
                    });
                }
            );
        }
...
fn bench_aggregate(c: &mut Criterion) {
    ...
    for num_clients in NUM_CLIENTS {
        for length in LENGTHS {
            let (params, _sk, cks) = Agg::setup(num_clients, length, &mut OsRng);
            let inputs = random_inputs(num_clients, length);
            let encodings: Vec<_> = cks.iter().zip(inputs.iter())
                .map(|(ck, input)| Agg::encode(ck, input, &mut OsRng).unwrap().0)
                .collect();
            group.bench_with_input(
                BenchmarkId::from_parameter(format!("{num_clients}_clients_{length}_inputs")),
                &(num_clients, length),
...
criterion_group!(benches, bench_setup, bench_encode, bench_verify, bench_aggregate, bench_decode, bench_post_process);
