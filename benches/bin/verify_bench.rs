...
impl TimeStats {
    fn from_times(times: &[Duration]) -> Self {
        ...
        let mean_nanos = times.iter().map(|d| d.as_nanos() as f64).sum::<f64>() / times.len() as f64;
...
fn print_results(results: &[(usize, TimeStats)], config: &Config, proof_system_name: &str) {
    ...
    println!("  Iterations: {} (warmup: {})", config.iterations, config.warmup);
    ...
    println!("  Clients | Mean (ms) | Per-User (ms) | Relative | Median (ms) | Min (ms) | Max (ms) | Std Dev (ms)");
    println!("  --------|-----------|---------------|----------|-------------|----------|----------|-------------");
